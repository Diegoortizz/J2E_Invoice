...
public class TransactionTest {
    ...
    public void productIdUnknown() throws SQLException {
        int id = myCustomer.getCustomerId();
        ...
        try {
            ...
            assertEquals(myDAO.numberOfInvoicesForCustomer(id), 0);
            throw e;
        }
        fail();
    }
    ...
    @Test(expected = SQLException.class)
    public void productIdUnknownAutreTest() throws SQLException {
        int id = myCustomer.getCustomerId();
        float before = myDAO.totalForCustomer(id);

        int[] productIds = new int[]{0, 1, 2, 5}; // le produit avec l'id 5 n'existe pas
        int[] quantities = new int[]{2, 1, 1, 1};
    ...
    @Test(expected = SQLException.class)
    public void quantiteNegative() throws SQLException {
        int[] productIds = new int[]{4};
        int[] quantities = new int[]{-2};
        myDAO.createInvoice(myCustomer, productIds, quantities);
    }

    @Test
    public void factureTest() throws SQLException {
        int id = myCustomer.getCustomerId();
        float before = myDAO.totalForCustomer(id);
        System.out.printf("Before: %f %n", before);

        // Un tableau de 1 productID
        int[] productIds = new int[]{0, 1, 2, 3}; // Le produit 0 vaut 10 €
        // Un tableau de 1 quantites
        int[] quantities = new int[]{10, 5, 2, 5};
        // On exécute la transaction
        myDAO.createInvoice(myCustomer, productIds, quantities);
        ...
        // Le client a maintenant 2*10€ de plus
        assertEquals(before + 10f * 10f + 20f * 5f + 2f * 30f + 5f * 40f, after, 0.001f);
